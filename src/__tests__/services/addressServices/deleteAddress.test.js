...
describe("testing delete address service", () => {
  test("should delete address and return addresses and status if API call succeeds", async () => {
    const data = {
      data: {
        address: [
          {
  ...
  test("should return error message from error object if API call fails", async () => {
    ...
    const response = await deleteAddressService({ _id, token });
